...
class MainFragment:Fragment(R.layout.fragment_main) {
    ...
    var time = 0.0
    // Вид медитации
    private lateinit var meditationKind: Meditation
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        random.setOnClickListener {
            ...
            animation()
                if (model.attempt == 1) {
                    attempt.text = "Осталась одна попытка"
            }
            if (model.attempt == 0) {
        ...
        start.setOnClickListener {
            ...
            bundle.putString("meditation", meditationKind.meditation)
    ...
    private fun animation(){
        ...
        meditationTextView.text = meditationKind.meditation
        meditationDescription.text = meditationKind.description
        time = (2..12).random().toDouble()
        timer.text = (time.toString() + "минут")
    }
}